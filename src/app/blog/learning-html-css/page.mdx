...
<PublicationDate date="2025-01-16T00:00:00Z" />
...
<Callout
  type="tip"
  title="If you're as new as I am, start small."
  content="Even making a simple page look cool with basic styling can spark major motivation."
/>
...
| --------- | ------------------------------------------------------------- | ------------------------------- | ----------------------------------------------------------- |
| Monday    | Basic tags: `<html>{:html}`, `<head>{:html}`, `<body>{:html}` | Inline styles                   | Felt odd mixing styles with HTML, but it was good practice. |
| Tuesday   | Headings & paragraphs                                         | External CSS w/ `<link>{:html}` | Loved keeping my CSS separate. Much cleaner.                |
...
I managed to build a tiny website for my personal notes. It only has a home page and a second page listing my to-do items, but it's fully styled! A small win, but it felt huge to me. Using `<div>{:html}`s and basic Flexbox properties, I was able to line up everything nicely.
